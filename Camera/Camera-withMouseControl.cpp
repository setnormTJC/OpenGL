...
/*yaw, pitch, lastX, and lastY get updated by mouse_callback*/
float yaw = -90.0f;	// yaw is initialized to -90.0 degrees since a yaw of 0.0 results in a direction vector pointing to the right so we initially rotate a bit to the left.
float pitch = 0.0f;
float lastX = 800.0f / 2.0;
float lastY = 600.0 / 2.0;

/*fov gets updated by scroll_callback*/
float fov = 45.0f;
...
#pragma region intiializing OpenGL
    ...
    glfwSetFramebufferSizeCallback(window, framebuffer_size_callback); //note the function ptr

    glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED); 
    //this prevents mouse from levaving window (dangeruese)

    glfwSetCursorPosCallback(window, mouse_callback); //for looking with the mouse 
...
#pragma region the render loop 
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (int i = 0; i < cubePositions.size(); i++) //loop makes more than one cube (multiple draw calls) 
        {
            ...
            lastFrame = currentFrame; //update for camera movement 
...
void processInput(GLFWwindow* window)
{
    ...
    float cameraSpeed = 1'000.0f * deltaTime; //increase constant for better-performing machines
    //std::cout << deltaTime << "\n";
    ...
    /*my own addition with up/down keys*/
    ...
    /*and resetting to default view (center) */
